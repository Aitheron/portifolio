...
import {resolveImageSource} from "@/lib/portfolio-types";
...
export function useProjectCoverTexture(
  ...
  const imageSrc = image ? resolveImageSource(image, locale) : undefined;
  const variants = imageSrc ? getImageVariants(imageSrc) : undefined;
  const previewSrc = variants?.thumbnail.src ?? imageSrc;
  const fullSrc = variants?.full.src ?? previewSrc;
  const identity = `${node.id}:${locale}:${imageSrc ?? "fallback"}`;
  ...
  useEffect(() => {
    ...
    const request = async () => {
      ...
      if (!cover && mounted && imageSrc && previewSrc !== imageSrc) {
        const lease = cache.acquire(`image:${imageSrc}`, () => loadImageTexture(imageSrc));
        leases.push(lease);
  ...
  }, [active, gl, identity, imageSrc, locale, node, previewSrc]);
